...
export class DadosComponent implements OnInit, OnDestroy {
  ...
  onSubmit() {
    ...
    if (this.onTestCampos() || (1 === 1)) {
      ...
      if (!this.atualizar) {
        this.buscarLacre.salvar(this.lacre).subscribe(() => { }, error => {
          this.avisocamposservice.mudarAviso(4);
          this.matsnackbarService.openSnackBarCampos(AvisocamposComponent, 1000);
        });
      }
      ...
      if (this.atualizar) {
        this.buscarLacre.atualizar(this.lacre).subscribe(() => { }, error => {
          this.avisocamposservice.mudarAviso(4);
          this.matsnackbarService.openSnackBarCampos(AvisocamposComponent, 1000);
        });
      }
  ...
  //#endregion
  criaArrayAtualização(lacre: Lacre[]) {
    go(lacre);
  }
